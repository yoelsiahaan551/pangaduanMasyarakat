...
const CATEGORY_MAP = {
  jalan:      { id: 1, label: "Jalan & Infrastruktur",  icon: <FaRoad />,      color: "#f59e0b" },
  lampu:      { id: 2, label: "Penerangan Jalan",        icon: <FaLightbulb />, color: "#3b82f6" },
  lingkungan: { id: 3, label: "Lingkungan & Taman",      icon: <FaLeaf />,      color: "#10b981" },
  air:        { id: 4, label: "Drainase & Air",          icon: <FaWater />,     color: "#06b6d4" },
  sampah:     { id: 5, label: "Kebersihan & Sampah",     icon: <FaTrash />,     color: "#8b5cf6" },
  fasilitas:  { id: 6, label: "Fasilitas Umum",          icon: <FaBuilding />,  color: "#ef4444" },
}

const PRIORITY_LEVELS = [
  { id: "rendah", label: "Rendah", desc: "Tidak mendesak, bisa ditangani dalam 30 hari" },
  { id: "sedang", label: "Sedang", desc: "Perlu perhatian, tangani dalam 14 hari" },
  { id: "tinggi", label: "Tinggi", desc: "Mendesak, perlu ditangani dalam 3 hari" },
]
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {PRIORITY_LEVELS.map((item) => (
                ...
                >
                  ...
                  <p className={`text-xs leading-relaxed ${priority === item.id ? "text-white/70" : "text-slate-400"}`}>
                    {item.desc}
                  </p>
                </button>
              ))}
